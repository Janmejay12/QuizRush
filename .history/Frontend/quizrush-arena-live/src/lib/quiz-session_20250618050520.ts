...
import { Quiz } from './types';
  ...
  export const quizSessionService = {
    ...
    resetQuiz: async (quizId: number): Promise<void> => {
      const hostId = localStorage.getItem('userId');
      if (!hostId) {
        throw new Error('Host ID not found. Please ensure you are logged in.');
      }
      ...
      try {
        await api.post(`/quiz-sessions/${quizId}/reset?hostId=${hostId}`);
      } catch (error: any) {
        console.error("Error resetting quiz:", error.response?.data || error);
        throw error;
      }
    },
