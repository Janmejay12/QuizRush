...
// Updated to match your backend LeaderboardDTO structure
...
export interface LeaderboardDTO {
  ...
  final: boolean; // Changed from isFinal to match backend
}
...
class WebSocketService {
  ...
  public subscribeToQuiz(roomCode: string, callbacks: {
    ...
    onLeaderboardUpdate?: (leaderboard: LeaderboardDTO) => void;
    ...
    return this.connect().then(() => {
          ...
          try {
            ...
            switch (parsedMessage.type) {
              case WebSocketMessageType.PARTICIPANT_JOINED:
                console.log('[WebSocket] Handling PARTICIPANT_JOINED');
            ...
            console.error('[WebSocket] Original message:', message.body);
            if (callbacks.onError) {
      ...
      // Request initial leaderboard state
      this.stompClient.publish({
        destination: `/app/quiz/${roomCode}/leaderboard`,
        body: JSON.stringify({}),
