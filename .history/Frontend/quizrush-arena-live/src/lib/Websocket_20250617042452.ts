...
export interface WebSocketMessage<T> {
  ...
  payload: T;
  roomCode: string;
  timestamp: number; // Note: your backend sends 'timeStamp' but this should be 'timestamp'
}
...
// Leaderboard data interface (matching your backend - using 'final' not 'isFinal')
export interface LeaderboardData {
  entries: LeaderboardEntry[];
  final: boolean; // Changed from isFinal to final to match backend
}
...
class WebSocketService {
  ...
  private resetConnection(): void {
    ...
    // Don't clear subscriptions here - let them be managed by the calling code
  }
  ...
  public subscribeToQuiz(roomCode: string, callbacks: {
    ...
    return this.connect().then(() => {
      ...
      // Check if already subscribed - don't unsubscribe unless necessary
      if (this.subscriptions[destination]) {
        console.log(`Already subscribed to ${destination}, keeping existing subscription`);
        return;
      }
          ...
          try {
            ...
            switch (parsedMessage.type) {
              case WebSocketMessageType.PARTICIPANT_JOINED:
                console.log('Processing PARTICIPANT_JOINED');
  ...
  // Unsubscribe from a quiz room - only call this when you actually want to stop listening
  ...
  public isSubscribedToRoom(roomCode: string): boolean {
    const destination = `/topic/quiz/${roomCode}`;
