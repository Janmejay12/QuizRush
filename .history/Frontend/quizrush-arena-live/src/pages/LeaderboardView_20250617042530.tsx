...
import { LeaderboardEntry, websocketService } from '@/lib/Websocket'; // Keep original casing
import { toast } from 'sonner';
import { LeaderboardData } from '@/lib/Websocket'; // Keep original casing
...
const LeaderboardView: React.FC = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        await websocketService.subscribeToQuiz(roomCode, {
          ...
          onLeaderboardUpdate: (leaderboard: LeaderboardData) => {
            ...
            addDebugLog(`Is final standings: ${leaderboard.final}`); // Changed from isFinal to final
    ...
    return () => {
      ...
      // Only unsubscribe when component is actually unmounting permanently
      // Don't unsubscribe on every re-render or navigation within the quiz
      const shouldUnsubscribe = !window.location.pathname.includes(`/quiz/${quizId}`) && 
                                 !window.location.pathname.includes(`/participant-quiz/${quizId}`) &&
                                 !window.location.pathname.includes(`/leaderboard/${quizId}`);
      
      if (roomCode && shouldUnsubscribe) {
        addDebugLog('Unsubscribing from WebSocket as component is permanently unmounting');
        websocketService.unsubscribeFromQuiz(roomCode);
      } else {
        addDebugLog('Keeping WebSocket subscription as we might return to this quiz');
      }
