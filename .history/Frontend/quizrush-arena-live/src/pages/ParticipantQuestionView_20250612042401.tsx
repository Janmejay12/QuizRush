...
const ParticipantQuestionView: React.FC = () => {
  ...
  const { data: currentQuestion, isLoading } = useQuery({
    ...
    cacheTime: 0 // Don't cache between component mounts
  });
  ...
  // Cleanup function for navigation timeouts and cache
  useEffect(() => {
    return () => {
      ...
      // Clear question cache if we're navigating away and not to another question
      if (!hasNavigatedRef.current && quizId) {
  ...
  useEffect(() => {
    ...
    console.log('🔄 Setting up WebSocket connection...', { quizId, participantId, pathname: location.pathname });
    ...
    const connectWebSocket = async () => {
      try {
        ...
        console.log('🔔 Subscribing to quiz room:', roomCode);
        await websocketService.subscribeToQuiz(roomCode, {
          onNewQuestion: (question: QuestionDTO) => {
            if (!isSubscribed) {
              console.log('❌ Ignoring new question - component unsubscribed');
              return;
            }
            ...
            console.log('💾 Updated question cache');
            ...
            setShowConfetti(false);
            setIsAnswerCorrect(null);
            console.log('🔄 Reset question states');
