...
const ParticipantWaitingRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = async () => {
      try {
        ...
        await websocketService.subscribeToQuiz(roomCode, {
          onQuizStarted: () => {
            if (!isSubscribed) {
              console.log('❌ Ignoring quiz start - component unsubscribed');
              return;
            }

            console.log('🎮 Quiz started! Navigating to question view...');
    ...
    return () => {
      isSubscribed = false;
      // Note: We don't unsubscribe here to maintain the connection during navigation
    };
