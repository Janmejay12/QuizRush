...
import { LeaderboardDTO } from '@/lib/types';
...
const LeaderboardView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('Initial mount - stored values:', { roomCode, participantId, quizId });
    ...
    const connectWebSocket = async () => {
      try {
        ...
        console.log('WebSocket connected, subscribing to quiz:', roomCode);

        await websocketService.subscribeToQuiz(roomCode, {
          onNewQuestion: (question) => {
            if (!isSubscribed) return;
            console.log('Received new question, navigating...');
          ...
          onLeaderboardUpdate: (leaderboard: LeaderboardDTO) => {
            console.log('LEADERBOARD_UPDATE received with payload:', leaderboard);
            if (!isSubscribed) return;
            
            const leaderboardData = leaderboard.LeaderBoardEntryDTO || leaderboard;
            ...
            console.log('Found participant in leaderboard:', participant);
            ...
            const entries = leaderboardData.entries.map(entry => ({
              ...
              score: entry.score,
              timeTaken: entry.totalTimeSpent
            }));
            console.log('Transformed leaderboard entries:', entries);
            setLeaderboardData(entries);
            setTotalParticipants(leaderboardData.entries.length);
            setIsFinalStandings(leaderboardData.final || leaderboardData.isFinal);
          },
          onQuizEnded: () => {
            if (!isSubscribed) return;
            console.log('Quiz ended, navigating to summary...');
        ...
        console.error('WebSocket connection/subscription error:', error);
        toast.error('Connection error. Please refresh the page.', {
          position: 'top-center'
        });
  ...
  // Track leaderboard data changes
  useEffect(() => {
    console.log('Leaderboard data updated:', leaderboardData);
  }, [leaderboardData]);

  // Track showLeaderboard changes
  useEffect(() => {
