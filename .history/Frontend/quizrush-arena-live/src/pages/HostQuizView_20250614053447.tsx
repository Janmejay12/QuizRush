...
const HostQuizView: React.FC = () => {
  ...
  const nextQuestionMutation = useMutation({
    ...
    onSuccess: (newQuestion) => {
      ...
      // Don't set timeLeft here, let the useEffect handle it
      // Don't set canProceed here either, let the useEffect handle it
  ...
  }, [currentQuestion?.id, isQuizEnded]); // Use currentQuestion.id to detect actual question changes

  // Timer effect
  useEffect(() => {
    if (!currentQuestion || isQuizEnded || timeLeft === null || timeLeft < 0) return;
